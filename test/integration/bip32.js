...
describe('bitbull-js (BIP32)', function () {
  it('can create a BIP32 wallet external address', function () {
    var path = "m/0'/0/0"
    var root = bitbull.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, bitbull, account 0, external address', function () {
  ...
  it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
    function recoverParent (master, child) {
      ...
      for (var i = 0; i < bitbull.HDNode.HIGHEST_BIT; ++i) {
        ...
        var Qp = new bitbull.ECPair(d2).Q
        if (Qp.equals(QP)) break
      }

      var node = new bitbull.HDNode(new bitbull.ECPair(d2), master.chainCode, master.network)
    ...
    var master = bitbull.HDNode.fromSeedBuffer(seed)
